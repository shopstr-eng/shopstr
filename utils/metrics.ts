...
const sortByDate = (data: Metrics[]) => {
  return data.sort((a, b) => (DateTime.fromISO(a.period).toMillis() - DateTime.fromISO(b.period).toMillis()))
};

const createDaysFromRange = (startDate: string, endDate: string) => {
  const days: string[] = [];

  for (let i = DateTime.fromISO(startDate); i < DateTime.fromISO(endDate); i = i.plus({ days: 1 })) {
...
const formatDataWithEmptyHours = (data: Metrics[], label: string, startDate: string) => {
  ...
  for (let i = 0; i <= finalHour; i++) {
    if (!newData.some((d) => DateTime.fromISO(d.period).hour === i)) {
      const dateTime = DateTime.fromISO(startDate).set({ hour: i, minute: 0, millisecond: 0 });
      newData.push({
  ...
  newData = newData.map((x) => ({ ...x, period: DateTime.fromISO(x.period).toFormat(HOUR_FORMAT) })) as Metrics[];
  return newData;
};

const formatDataWithEmptyDays = (data: Metrics[], label: string, startDate: string, endDate: string) => {
  const newData = [...data.map(x => ({ ...x, period: DateTime.fromISO(x.period).toFormat(DATE_FORMAT) }))] as Metrics[];
...
export const formatDataWithEmptyDateTime = (data: Metrics[], label: string, startDate: string, endDate: string) => {
