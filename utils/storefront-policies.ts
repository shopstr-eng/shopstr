...
export const POLICY_KEYS: (keyof StorefrontPolicies)[] = [
  "returnPolicy",
  "termsOfService",
  "privacyPolicy",
  "cancellationPolicy",
...
${name} operates as an independent seller on the Shopstr platform. By placing an order with ${name}, you agree to these terms.
...
All payments are processed through the Shopstr platform using Bitcoin Lightning, Cashu ecash, or other accepted payment methods. Prices are displayed in the currency shown on each listing. Once a payment is confirmed on the network, it is considered final.
...
If you have a concern about an order, please contact us directly. We are committed to resolving issues fairly and promptly. As peer-to-peer transactions on the Shopstr platform, disputes are resolved directly between buyer and seller.
