...
describe("replaceable event ordering", () => {
  ...
  it("coerces string created_at values before comparing", () => {
    const olderFromDb = { id: "0".repeat(64), created_at: "100" };
    const newerFromRelay = { id: "f".repeat(64), created_at: 101 };

    expect(pickPreferredReplaceableEvent([olderFromDb, newerFromRelay])).toBe(
      newerFromRelay
    );
  });
});
