 ...
 * `LookupInvoiceV2`, `SettleInvoice`, `CancelInvoice`. The connection setup
...
export class LndHodlInvoiceProvider implements HodlInvoiceProvider {
  ...
  async lookupInvoice(paymentHash: string): Promise<LookupInvoiceResult> {
    ...
    // Guards against reading a different invoice than the one asked for — a
    // mixed-up hash here would report someone else's escrow as settled.
    const returnedHash = bytesToHex(invoice?.r_hash);
    if (returnedHash !== undefined && returnedHash !== normalized) {
      throw new LndProviderError(
    ...
    if (!status) {
      // Unrecognised rather than mapped to a guess: reporting an unknown state
      // as `open` would invite a cancel of funds that are actually held, and
      // as `settled` would release an order nobody paid.
      throw new LndProviderError(
