...
jest.mock('../../display-products', () => function MockDisplayProducts() { return <div data-testid="display-products-mock" />; });
jest.mock('../../sign-in/SignInModal', () => function MockSignInModal() { return <div data-testid="signin-modal-mock" />; });
jest.mock('../../home/side-shop-nav', () => function MockSideShopNav() { return <div data-testid="side-shop-nav-mock" />; });
jest.mock('@braintree/sanitize-url', () => ({ sanitizeUrl: (url: string) => url }));
...
describe('MyListingsPage', () => {
  ...
  describe('When user is logged out', () => {
    ...
    test('does not render user-specific content', () => {
      expect(screen.queryByTestId('display-products-mock')).not.toBeInTheDocument();
      expect(screen.queryByTestId('side-shop-nav-mock')).not.toBeInTheDocument();
    ...
    test('opens sign-in modal when trying to view "Orders"', () => {
      const desktopOrdersButton = screen.getAllByRole('button', { name: 'Orders' })[0];
  ...
  describe('When user is logged in', () => {
    describe('and has a shop profile', () => {
      ...
      test('fetches and displays shop banner and about info', () => {
        ...
        expect(screen.getByAltText('Shop Banner')).toHaveAttribute('src', shopProfile.content.ui.banner);
    ...
    describe('and has no shop profile', () => {
        ...
        test('shows "Nothing here" message in About section', () => {
            ...
            expect(screen.getByText('Set up your shop in settings!')).toBeInTheDocument();
        });
    });
  ...
  describe('Navigation and UI', () => {
    ...
    test('switches between Listings and About sections', () => {
      expect(screen.getByTestId('display-products-mock')).toBeInTheDocument();
      expect(screen.queryByText(shopProfile.content.about)).not.toBeInTheDocument();
    ...
    test('mobile menu opens and closes on click', () => {
      const menuButton = screen.getAllByTestId('bars3-icon-mock')[0].closest('button');
      expect(menuButton).toBeInTheDocument();
      
      expect(screen.queryByText('Listings', { selector: '.absolute button' })).toBeNull();

      fireEvent.click(menuButton!);
      expect(screen.getByText('Listings', { selector: '.absolute button' })).toBeInTheDocument();
      expect(screen.getByText('About', { selector: '.absolute button' })).toBeInTheDocument();
