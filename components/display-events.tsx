...
const DisplayEvents = ({
  ...
  useEffect(() => {
    ...
    let productsSub = pool.sub(getRelays(), [subParams]);
    productsSub.on('event', (event) => {
      setEventData((eventData) => {
        let newEventData = [...eventData, event as Event];
            ...
            <div className="mt-2 text-gray-800 text-sm md:text-base whitespace-pre-wrap break-words">
              ...
              } */}
              {
                event.kind == 30402 ? (
                  ...
                  event.content.search(imageUrlRegExp) ? (
                    <div>
                      <p>{event.content.replace(imageUrlRegExp, '')}</p>
                      { /* eslint-disable-next-line @next/next/no-img-element */ }
                      <img src={event.content.match(imageUrlRegExp)?.[0] || ''} alt="Product Image" />
                    </div>
                  ) : (
                    <div>
                      <p>{event.content}</p>
