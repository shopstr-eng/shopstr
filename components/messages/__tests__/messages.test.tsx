...
jest.mock("../chat-panel", () => {
	return function MockChatPanel({ handleSendMessage, isSendingDMLoading }: any) {
		return (
...
describe("Messages Component", () => {
	...
	const mockChatsMap = new Map([
		[
			mockChatPubkey1,
			[
				{ id: "msg1", kind: 14, content: "Hello", pubkey: mockChatPubkey1, created_at: 1000, tags: [["subject", "listing-inquiry"]], read: true },
				{ id: "msg2", kind: 14, content: "New Message", pubkey: mockUserPubkey, created_at: 1002, tags: [["subject", "listing-inquiry"]], read: false },
			],
		],
		[
			mockChatPubkey2,
			[
				{ id: "msg3", kind: 14, content: "Hi there", pubkey: mockChatPubkey2, created_at: 999, tags: [["subject", "listing-inquiry"]], read: true },
			],
		],
	]);
	...
	beforeEach(() => {
		...
		mockNostrHelper.generateKeys.mockResolvedValue({ nsec: "testnsec", npub: "testnpub" });
		mockNostrHelper.decryptNpub.mockImplementation((npub) => `${npub}-decrypted`);
	...
	it("should render a sign-in prompt when not logged in", async () => {
		...
		renderComponent();
		await waitFor(() => {
			expect(screen.getByText("You must be signed in to see your chats!")).toBeInTheDocument();
		});
	});
	...
	it("should handle a pubkey from router query and open the chat", async () => {
		...
		renderComponent();
		await waitFor(() => {
			expect(screen.getByTestId("chat-button-new_chat_npub-decrypted")).toBeInTheDocument();
		});
	});
	...
	it("should enter a chat and mark messages as read when a ChatButton is clicked", async () => {
		...
		renderComponent();
		await waitFor(() => {
			expect(screen.getByTestId(`chat-button-${mockChatPubkey1}`)).toBeInTheDocument();
		});
		await act(async () => {
			fireEvent.click(screen.getByTestId(`chat-button-${mockChatPubkey1}`));
		});
		await waitFor(() => {
			const updatedMessages = mockChatsMap.get(mockChatPubkey1)!.map(m => ({ ...m, read: true }));
			expect(mockCacheService.addChatMessagesToCache).toHaveBeenCalledWith(updatedMessages);
		});
	});
	...
	it("should send a gift-wrapped message successfully", async () => {
		...
		const mockGiftWrappedEvent = { id: "new-gift-event", content: "Test message", kind: 14, pubkey: mockUserPubkey, created_at: 1005, tags: [['p', mockChatPubkey1]] };
		mockNostrHelper.constructGiftWrappedEvent.mockResolvedValue(mockGiftWrappedEvent as any);
		mockNostrHelper.constructMessageSeal.mockResolvedValue("sealed-event" as any);
		mockNostrHelper.constructMessageGiftWrap.mockResolvedValue("gift-wrapped-event" as any);
		...
		await waitFor(() => {
			expect(mockNostrHelper.constructGiftWrappedEvent).toHaveBeenCalledWith(mockUserPubkey, mockChatPubkey1, "Test message", "listing-inquiry");
		});
	});
	...
	it("should handle errors when sending a message", async () => {
		mockChatsContextValue.isLoading = false;
		mockChatsContextValue.chatsMap = mockChatsMap;
		mockNostrHelper.constructGiftWrappedEvent.mockRejectedValue(new Error("Send failed"));
