...
jest.mock('@/components/utility-components/profile/profile-avatar', () => ({
  ProfileAvatar: ({ pubkey, description }: { pubkey: string; description: string }) => (
...
describe('ChatButton Component', () => {
  ...
  test('renders correctly with message data and no unread count', () => {
    ...
    expect(unreadSpans.find(span => span.className.includes('rounded-full'))).toBeUndefined();
  ...
  test('applies active styles when it is the opened chat', () => {
    ...
    const container = screen.getByTestId('profile-avatar').closest('div.cursor-pointer');
  ...
  test('renders correctly when there are no messages', () => {
    ...
    const timeContainer = screen.getByText('No messages yet').closest('.flex')?.querySelector('.text-right > div:last-child > span');
