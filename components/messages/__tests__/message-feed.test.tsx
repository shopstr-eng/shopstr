...
jest.mock('../messages', () => {
  return function MockMessages({ isPayment }: { isPayment: boolean }) {
    return <div>{isPayment ? 'Orders Messages Content' : 'Inquiries Messages Content'}</div>;
  };
});
...
describe('MessageFeed Component', () => {
  ...
  test('renders with "Orders" as the default initial tab', () => {
    ...
    expect(mockUseTabs).toHaveBeenCalledWith(expect.objectContaining({ initialTabId: 'orders' }));
    expect(screen.getByText('Orders Messages Content')).toBeInTheDocument();
    expect(screen.queryByText('Inquiries Messages Content')).not.toBeInTheDocument();
  ...
  test('renders with "Inquiries" as the initial tab when isInquiry is true', () => {
    ...
    expect(mockUseTabs).toHaveBeenCalledWith(expect.objectContaining({ initialTabId: 'inquiries' }));
    expect(screen.getByText('Inquiries Messages Content')).toBeInTheDocument();
    expect(screen.queryByText('Orders Messages Content')).not.toBeInTheDocument();
  });
