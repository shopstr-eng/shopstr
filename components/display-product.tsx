...
import requestMint from '../pages/api/cashu/request-mint';
...
const DisplayProduct = ({ tags, eventId, pubkey, handleDelete }:
  { tags: ProductFormValues, eventId: string, pubkey: string, handleDelete: (productId: string) => void }) => {
  ...
  async function invoiceHasBeenPaid(pk: string, wallet: CashuWallet, price: number, hash: string) {
    let encoded;
    while (true) {
      try {
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="flex justify-center">
        <BoltIcon
          ...
          onClick={() => handleCheckout(eventId, pubkey, Number(price))}
        />
        {
          ...
          <div className="flex items-end justify-center min-h-screen text-center sm:block sm:p-0">
            ...
            {(!paymentConfirmed) ? (<div className="inline-block align-bottom bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="inline-block align-bottom bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <Image src="../payment-confirmed.gif" alt="Payment Confirmed" fill={true} />
              </div>
            )}
          </div>
        </div>
