...
const mockWalletContext = {
    proofEvents: [
        { id: 'event1', proofs: [{ id: '00d0a1b24d1c1a53', amount: 50, secret: 'secret1' }] },
        { id: 'event2', proofs: [{ id: '00d0a1b24d1c1a53', amount: 30, secret: 'secret2' }] },
    ],
...
describe('PayButton Component', () => {
    ...
    test('shows validation error for invalid invoice prefix', async () => {
        ...
        fireEvent.change(invoiceInput, { target: { value: 'this_is_an_invalid_invoice' } });
        fireEvent.click(submitButton);

        expect(await screen.findByText("The lightning invoice must start with 'lnbc'.")).toBeVisible();
    ...
    test('shows validation error for empty invoice', async () => {
        ...
        expect(await screen.findByText('A Lightning invoice is required.')).toBeVisible();
        expect(publishProofEvent).not.toHaveBeenCalled();
    });
    ...
    test('shows NIP46 signer information when using NostrNIP46Signer', async () => {
        ...
        expect(screen.queryByText(/If the invoice payment is taking a while/)).not.toBeInTheDocument();
    });
    ...
    test('handles payment with change proofs correctly', async () => {
        const mockInvoice = 'lnbc100n...';
        const changeProofs = [{ id: '00d0a1b24d1c1a53', amount: 20, secret: 'change_secret' }];
    ...
    test('handles a failed payment flow', async () => {
        ...
        mockCreateMeltQuote.mockResolvedValue({ amount: 100, fee_reserve: 2 });
        mockGetKeySets.mockResolvedValue([{ id: '00d0a1b24d1c1a53' }]);
        mockSend.mockResolvedValue({ keep: [], send: [{ id: '00d0a1b24d1c1a53', amount: 102, secret: 'mock_secret' }] });
        ...
        expect(publishProofEvent).not.toHaveBeenCalled();
        expect(localStorageMock.getItem('tokens')).toBe(JSON.stringify(initialTokens));
    });
    ...
    test('shows loading spinner during payment processing', async () => {
        ...
        mockMeltProofs.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({ paid: true, change: [] }), 100)));
