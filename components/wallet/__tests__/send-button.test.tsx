...
const renderWithProviders = (
  ...
  { signer = mockSigner, nostr = mockNostr, walletContext = mockWalletContext } = {}
) => {
  return render(
...
describe('SendButton', () => {
  ...
  test('shows validation errors for invalid input', async () => {
    ...
    expect(await within(modal).findByText('A whole number is required.')).toBeVisible();
  });

  test('handles a successful send transaction', async () => {
    const mockSendResult = { keep: [{id: 'keyset_id_1', amount: 900}], send: [{id: 'keyset_id_1', amount: 100}] };
    ...
    await userEvent.click(screen.getByRole('button', { name: /Send/i }));
    const modal = await screen.findByRole('dialog');
    await userEvent.type(within(modal).getByLabelText(/Token amount in sats/i), '100');
    await userEvent.click(within(modal).getByRole('button', { name: /Send/i }));

    await screen.findByText('New token string is ready to be copied and sent!');
    expect(localStorage.setItem).toHaveBeenCalledWith('history', expect.stringContaining('"type":2,"amount":"100"'));
  });

  test('handles send with no change proofs', async () => {
    const mockSendResult = { keep: [], send: [{id: 'keyset_id_1', amount: 100}] };
    mockSend.mockResolvedValue(mockSendResult);
  ...
  test('handles input validation for non-numeric values', async () => {
    ...
    expect(await within(modal).findByText('Please enter a whole number.')).toBeVisible();
  });
  ...
  test('handles input validation for values exceeding max length', async () => {
    ...
    expect(await within(modal).findByText('This input exceed maxLength of 500.')).toBeVisible();
  });
  ...
  test('resets form and states when modal is toggled', async () => {
    ...
    await userEvent.type(within(modal).getByLabelText(/Token amount in sats/i), '2000');
    await userEvent.click(within(modal).getByRole('button', { name: /Send/i }));
  ...
  test('handles tokens with different keyset IDs', async () => {
    ...
    MockCashuWallet.mockImplementation(() => ({
      getKeySets: jest.fn().mockResolvedValue([
        { id: 'keyset_id_1' }, 
        { id: 'keyset_id_2' }
      ]),
  ...
  test('handles a failed send transaction', async () => {
    ...
    expect(await screen.findByText('Send failed!')).toBeVisible();
    expect(screen.getByText(/You don't have enough funds to send/)).toBeVisible();
  });
  ...
  test('shows NIP-46 info when using a bunker signer', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /Send/i }));
    const modal = await screen.findByRole('dialog');
    expect(within(modal).getByText(/If the token is taking a while to be generated/)).toBeVisible();
  });
});
