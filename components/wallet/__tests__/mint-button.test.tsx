...
jest.mock('@/components/utility-components/failure-modal', () => ({
    __esModule: true,
    default: ({ isOpen, bodyText, onClose }: { isOpen: boolean; bodyText: string; onClose: () => void }) =>
        isOpen ? (
            <div data-testid="failure-modal">
                <p>{bodyText}</p>
                <button onClick={onClose} data-testid="failure-modal-close">Close</button>
            </div>
        ) : null,
}));
...
jest.mock('@heroicons/react/24/outline', () => ({
    ...
    ClipboardIcon: (props: React.SVGProps<SVGSVGElement>) => <div data-testid="clipboard-icon" {...props} />,
...
const renderComponent = (customSigner = mockSigner) => {
    return render(
        <SignerContext.Provider value={{ signer: customSigner, setSigner: jest.fn() }}>
...
describe('MintButton Component', () => {
    ...
    it('should show validation error for invalid input', async () => {
        ...
        const submitButton = screen.getByRole('button', { name: /Mint/i, type: 'submit' });
        fireEvent.change(input, { target: { value: 'abc' } });
        fireEvent.click(submitButton);
        expect(await screen.findByText('Please enter a whole number.')).toBeVisible();
    });
    ...
    it('should handle the full successful minting process', async () => {
        ...
        mockCreateMintQuote.mockResolvedValue({ request: mockInvoice, quote: mockHash });
        ...
        fireEvent.click(screen.getByRole('button', { name: /Mint/i }));
        await waitFor(() => expect(screen.getByRole('dialog')).toBeVisible());
        fireEvent.change(screen.getByLabelText(/Token amount in sats/i), { target: { value: satsToMint } });
        fireEvent.click(screen.getByRole('button', { name: /Mint/i, type: 'submit' }));
    ...
    it('should handle payment timeout', async () => {
        const satsToMint = '50';
        mockCreateMintQuote.mockResolvedValue({ request: 'lnbc500...', quote: 'hash50' });
        mockCheckMintQuote.mockRejectedValue(new TypeError('Network connection failed'));
        ...
        expect(screen.getByText('Failed to validate invoice! Change your mint in settings and/or please try again.')).toBeVisible();
    });

    it('should handle max retries timeout', async () => {
        const satsToMint = '25';
        mockCreateMintQuote.mockResolvedValue({ request: 'lnbc250...', quote: 'hashTimeout' });
        ...
        expect(screen.getByText('Payment timed out! Please check your wallet balance or try again.')).toBeVisible();
        expect(callCount).toBeGreaterThan(1);
    });

    it('should handle copy to clipboard functionality', async () => {
        const mockInvoice = 'lnbc_very_long_invoice_string';
        mockCreateMintQuote.mockResolvedValue({ request: mockInvoice, quote: 'some_hash' });
   
        renderComponent();
        fireEvent.click(screen.getByRole('button', { name: /Mint/i }));
        await waitFor(() => expect(screen.getByRole('dialog')).toBeVisible());
        fireEvent.change(screen.getByLabelText(/Token amount in sats/i), { target: { value: '10' } });
        fireEvent.click(screen.getByRole('button', { name: /Mint/i, type: 'submit' }));
    ...
    it('should handle ISSUED state in invoice polling', async () => {
        ...
        expect(screen.getByText('Payment was received but your connection dropped! Please check your wallet balance.')).toBeVisible();
    });
