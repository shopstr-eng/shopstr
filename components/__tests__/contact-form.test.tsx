...
describe('ContactForm', () => {
  ...
  test('renders the modal with standard fields when showContactModal is true', () => {
    ...
    expect(screen.getByRole('textbox', { name: /Contact Contact/i })).toBeInTheDocument();
  });

  test('does not render the extra "Required" field if requiredInfo is not provided', () => {
    render(<TestWrapper />);
    expect(screen.queryByRole('textbox', { name: /Enter/i })).not.toBeInTheDocument();
  });
  ...
  test('renders the extra "Required" field when requiredInfo is provided', () => {
    ...
    expect(screen.getByRole('textbox', { name: `${expectedLabel} ${expectedLabel}` })).toBeInTheDocument();
  });
  ...
  describe('Form Submission and Validation', () => {
    test('shows validation errors when submitting an empty form', async () => {
      ...
      expect(await screen.findByText('A contact is required.')).toBeInTheDocument();
      expect(screen.getByText('A contact type is required.')).toBeInTheDocument();
      expect(screen.getByText('Delivery instructions are required.')).toBeInTheDocument();
      expect(onContactSubmit).not.toHaveBeenCalled();
    });

    test('shows a validation error for the conditional field', async () => {
      const onContactSubmit = jest.fn();
      render(<TestWrapper onContactSubmit={onContactSubmit} requiredInfo="PO Box" />);
      
      // Fill out the standard fields, but leave the conditional one empty
      fireEvent.change(screen.getByRole('textbox', { name: /Contact Contact/i }), { target: { value: 'test' } });
      fireEvent.change(screen.getByRole('textbox', { name: /Contact type Contact type/i }), { target: { value: 'test' } });
      fireEvent.change(screen.getByRole('textbox', { name: /Delivery instructions Delivery instructions/i }), { target: { value: 'test' } });
    ...
    test('submits the form with user input', async () => {
      ...
      fireEvent.change(screen.getByRole('textbox', { name: /Contact Contact/i }), { target: { value: 'user@example.com' } });
      fireEvent.change(screen.getByRole('textbox', { name: /Contact type Contact type/i }), { target: { value: 'email' } });
      fireEvent.change(screen.getByRole('textbox', { name: /Delivery instructions Delivery instructions/i }), { target: { value: 'Leave at front door.' } });
    ...
    test('submits the form including the extra required field', async () => {
      ...
      render(<TestWrapper onContactSubmit={onContactSubmit} requiredInfo={requiredLabel} />);
      const expectedDynamicLabel = `Enter ${requiredLabel}`;
      
      fireEvent.change(screen.getByRole('textbox', { name: /Contact Contact/i }), { target: { value: 'user2' } });
      fireEvent.change(screen.getByRole('textbox', { name: /Contact type Contact type/i }), { target: { value: 'nostr' } });
      fireEvent.change(screen.getByRole('textbox', { name: /Delivery instructions Delivery instructions/i }), { target: { value: 'DM for details.' } });
      fireEvent.change(screen.getByRole('textbox', { name: `${expectedDynamicLabel} ${expectedDynamicLabel}` }), { target: { value: '12345' } });
