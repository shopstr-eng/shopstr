...
jest.mock('@/components/utility-components/file-uploader', () => ({
  FileUploaderButton: jest.fn(({ children, imgCallbackOnUpload, isIconOnly }) => (
    <button
...
const mockShopData = new Map([
  [mockUserPubkey, {
    ...
    content: {
      ...
      ui: { picture: 'https://existing.image/picture.png', banner: 'https://existing.image/banner.png' },
    },
  }],
]);

const renderWithProviders = (component: React.ReactElement, shopData = new Map()) => {
  ...
  render(
    <NostrContext.Provider value={{ nostr: {} as any }}>
      <SignerContext.Provider value={{ signer: {} as any, pubkey: mockUserPubkey }}>
        <ShopMapContext.Provider value={{ shopData, updateShopData: mockUpdateShopData }}>
          {component}
...
describe('ShopProfileForm', () => {
  ...
  test('populates the form with existing shop data', async () => {
    ...
    expect(await screen.findByDisplayValue('My Awesome Shop')).toBeInTheDocument();
    ...
    expect(picture).toHaveAttribute('src', 'https://existing.image/picture.png');
  ...
  test('updates form values on file upload simulation', async () => {
    ...
    expect(await screen.findByAltText('shop logo')).toHaveAttribute('src', 'https://new.image/url');
  });
  ...
  test('submits the form, shows loading state, and calls relevant functions', async () => {
    ...
    mockCreateNostrShopEvent.mockImplementation(() => new Promise(resolve => {
  ...
  test('redirects after submission if isOnboarding is true', async () => {
    ...
    await user.type(await screen.findByLabelText('Shop Name'), 'Onboarding Shop');
  ...
  test('shows a validation error for inputs that exceed maxLength', async () => {
    ...
    const shopNameInput = await screen.findByLabelText('Shop Name');
    await user.type(shopNameInput, 'This is a very long shop name that is definitely over fifty characters long for sure.');
    await user.click(screen.getByRole('button', { name: /Save Shop/i }));
    
    expect(await screen.findByText('This input exceed maxLength of 50.')).toBeInTheDocument();
  });
