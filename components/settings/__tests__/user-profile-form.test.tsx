...
const renderWithProviders = (component: React.ReactElement, profileData = new Map(), pubkey: string | null = mockUserPubkey) => {
  ...
  render(
    <NostrContext.Provider value={{ nostr: {} as any }}>
      <SignerContext.Provider value={{ signer: {} as any, pubkey }}>
        <ProfileMapContext.Provider value={{ profileData, updateProfileData: mockUpdateProfileData }}>
          {component}
...
describe('UserProfileForm', () => {
  ...
  test('displays default image when no picture is available', async () => {
    ...
    expect(profileImage).toHaveAttribute('src', `https://robohash.org/${mockUserPubkey}`);
  });
  ...
  test('redirects after submission if isOnboarding is true', async () => {
    ...
    await user.type(await screen.findByLabelText('Display name'), 'Onboarding User');
    await user.click(screen.getByRole('button', { name: /Save Profile/i }));
  ...
  test('handles fiat payment option changes correctly', async () => {
    ...
    const venmoInput = await screen.findByPlaceholderText('Enter your Venmo username/tag');
    ...
    expect(screen.queryByPlaceholderText('Enter your Venmo username/tag')).not.toBeInTheDocument();
  });
