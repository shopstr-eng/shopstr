...
- **Seller Subdomain Storefronts**: Sellers can set up customizable standalone shop pages accessible via `/shop/[slug]`, subdomains (`slug.milk.market`), or custom domains. Storefront customization includes: color scheme (primary, secondary, accent, background, text colors with 7 presets), landing page style (hero/classic/minimal), and product layout (grid/list/featured). Custom domains require CNAME DNS records pointing to milk.market. Configuration stored in `storefront` field of Kind 30019 shop profile content JSON (`StorefrontConfig` type with `colorScheme`, `productLayout`, `landingPageStyle`, `shopSlug`, `customDomain`). Database tables: `shop_slugs` (pubkey-to-slug mapping), `custom_domains` (pubkey-to-domain mapping with verification status). API routes: `pages/api/storefront/lookup.ts` (slug/domain lookup), `pages/api/storefront/register-slug.ts` (slug registration with reserved word protection), `pages/api/storefront/custom-domain.ts` (domain CRUD). Storefront pages: `pages/shop/[slug].tsx` (slug-based), `pages/shop/_custom-domain.tsx` (custom domain resolution). Components: `components/storefront/storefront-layout.tsx` (main layout with CSS variable theming), `components/storefront/storefront-hero.tsx` (hero landing style), `components/storefront/storefront-product-grid.tsx` (product display with layout variants). Subdomain routing handled in `proxy.ts` (rewrites `slug.milk.market` to `/shop/slug`). "Visit Storefront" link added to marketplace seller pages and product card profile dropdowns. Settings UI for all storefront options added to `components/settings/shop-profile-form.tsx`.
