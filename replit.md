...
- **Seller Subdomain Storefronts**: Sellers can set up customizable standalone shop pages accessible via `/shop/[slug]`, subdomains (`slug.milk.market`), or custom domains. Storefront customization includes: color scheme (primary, secondary, accent, background, text colors with 7 presets), landing page style (hero/classic/minimal), product layout (grid/list/featured), Google Fonts selection (heading + body), section-based page builder, multi-page navigation, and custom footer. Configuration stored in `storefront` field of Kind 30019 shop profile content JSON (`StorefrontConfig` type). Database tables: `shop_slugs` (pubkey-to-slug mapping), `custom_domains` (pubkey-to-domain mapping with verification status). API routes: `pages/api/storefront/lookup.ts`, `pages/api/storefront/register-slug.ts`, `pages/api/storefront/custom-domain.ts`. Storefront pages: `pages/shop/[slug].tsx` (main page), `pages/shop/[...shopPath].tsx` (catch-all for sub-pages). Components: `components/storefront/storefront-layout.tsx` (main layout with CSS variable theming and Google Fonts), `components/storefront/storefront-nav.tsx` (navigation bar), `components/storefront/storefront-footer.tsx` (custom footer with social links), `components/storefront/section-renderer.tsx` (dispatches sections to components), `components/storefront/sections/` (12 section components: hero, about, story, products, testimonials, faq, ingredients, comparison, text, image, contact, reviews). Section editor UI in `components/settings/storefront/` (section-editor.tsx, footer-editor.tsx, page-editor.tsx) integrated into shop profile settings. Section-based layout activates when `storefront.sections` array has entries; otherwise falls back to legacy hero/classic/minimal + product grid. Multi-page support via `storefront.pages` array, each page with its own sections and URL slug. Subdomain routing handled in `proxy.ts`. MCP tools updated: `set_shop_profile` accepts fonts, sections, pages, footer, navLinks, showCommunityPage, showWalletPage; `get_storefront` returns full config including community/wallet toggles. **Storefront My Listings Page**: A "My Listings" nav link appears only when the signed-in user's pubkey matches the shop owner's pubkey. Route `/shop/[slug]/my-listings` renders `components/storefront/storefront-my-listings.tsx`, which wraps `MyListingsFeed` with storefront styling. Non-owners see a "Page Not Found" message — the link doesn't appear in nav and the page blocks access. No toggle needed. **Storefront Wallet Page**: When `showWalletPage` is enabled in `StorefrontConfig`, a "Wallet" link is added to the storefront nav. Route `/shop/[slug]/wallet` renders `components/storefront/storefront-wallet.tsx`, which provides the full Cashu ecash wallet (receive, send, mint, pay) styled with the storefront's colors. The balance card and transactions card use storefront colors instead of the default neo-brutalist theme. Toggle is in the shop profile settings form labeled "Show Bitcoin Wallet Page". **Storefront Orders Page**: An "Orders" link is always present in the storefront nav (no toggle needed). Route `/shop/[slug]/orders` renders `components/storefront/storefront-orders.tsx`, which wraps `OrdersDashboard` with storefront color styling. Requires sign-in — shows a prompt if not logged in. The `StorefrontThemeWrapper` mobile menu also includes an "Orders" link so users can reach it from listing/cart pages. **Storefront Community Page**: When `showCommunityPage` is enabled in `StorefrontConfig`, a "Community" link is auto-added to the storefront nav. The route `/shop/[slug]/community` renders `components/storefront/storefront-community.tsx`, which finds the seller's community (Kind 34550 event matching their pubkey) from `CommunityContext` and displays the community header/description plus `CommunityFeed` — all styled with the storefront's colors. Toggle is in the shop profile settings form. **Storefront Theme Wrapper**: `components/storefront/storefront-theme-wrapper.tsx` extends seller branding to the product detail page (`pages/listing/[[...productId]].tsx`) and cart page (`pages/cart/index.tsx`). When a product belongs to a seller with a custom storefront, the wrapper: (1) renders a full branded nav bar (fixed top, z-50) with shop logo/name linking to the shop home, a cart icon with badge, sign-in button (or profile dropdown when logged in), and mobile hamburger menu; (2) hides the main Milk Market TopNav via `body.sf-active [data-main-nav]` CSS rule (TopNav has `data-main-nav` attribute); (3) applies CSS variable overrides for colors, fonts, borders, shadows both inside `.storefront-themed` div AND in NextUI portal containers (`body.sf-active [data-overlay-container]`) so sign-in modals and other portaled UI also get themed; (4) renders the seller's custom footer; (5) stores the seller pubkey in `sessionStorage.sf_seller_pubkey` so the cart page can pick it up and wrap itself with the same theme. Falls back to default Milk Market styling if no storefront config exists. **Preview Placeholders**: The storefront preview modal fills realistic placeholder content for empty section fields so sellers see a fully populated preview. **Storefront Cart Filtering**: When viewing the cart from a storefront context (`sessionStorage.sf_seller_pubkey` is set), only products from that seller are shown. Items from other sellers are hidden with a notice showing how many excluded items exist and a "View full cart" link that clears the storefront context and shows all items. The full `localStorage` cart is never modified — filtering is display-only. The `handleRemoveFromCart` function properly maintains the filter when removing items. **Storefront Order Confirmation Page**: Route `/shop/[slug]/order-confirmation` renders `components/storefront/storefront-order-confirmation.tsx` — a branded confirmation page using storefront colors, showing order details (items, payment method, shipping, totals), delivery info, and action buttons (Continue Shopping, Check Order Status, Contact Merchant) that link back to the storefront. Both cart and single-product checkout pages redirect to this storefront confirmation page when `sf_seller_pubkey` and `sf_shop_slug` are set in sessionStorage, otherwise they redirect to the standard `/order-summary` page. The `sf_shop_slug` is stored in sessionStorage by both `StorefrontLayout` and `StorefrontThemeWrapper`.
