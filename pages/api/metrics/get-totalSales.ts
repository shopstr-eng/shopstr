...
export default async function GetTotalSales(req: NextApiRequest, res: NextApiResponse) {
  ...
  const transactions = await repo().raw(`SELECT time_bucket('1 day', time) AS bucket, sum(total) AS value FROM invoices WHERE time BETWEEN '${startDate}' AND '${endDate}' AND status = 'PAID' AND merchant_id = '${merchantId}' GROUP BY bucket ORDER BY bucket DESC;`)
