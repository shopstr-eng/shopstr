...
export default async function GetUniqueCustomers(req: NextApiRequest, res: NextApiResponse) {
  ...
  const transactions = await repo().raw(`SELECT time_bucket('1 day', time) AS bucket, count(customer_id) AS value FROM invoices WHERE time BETWEEN '${startDate}' AND '${endDate}' AND merchant_id = '${merchantId}' GROUP BY bucket ORDER BY bucket DESC;`)
