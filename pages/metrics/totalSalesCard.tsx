...
export default function TotalSalesCard({ startDate, endDate, interval }:
  ...
  useEffect(() => {
    ...
    fetch(`/api/metrics/get-totalSales?startDate=${startDate}&endDate=${endDate}&merchantId=${merchantId}`)
      .then((res) => res.json())
      .then((data) => {
        ...
        setTotalSales(formattedData.reduce((acc, curr) => acc + Number(curr.value), 0))
